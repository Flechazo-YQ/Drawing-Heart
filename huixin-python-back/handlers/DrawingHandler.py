...
from mongodb_config import drawing_analysis_manager
...
class DrawingHandler:
    ...
    def saveDrawing(cls):
        ...
        userId = UserTokenHandler.verifyToken(token)
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    return flask.jsonify({'message': f'图像数据解码失败: { str(decode_error) }'}), 400
                ...
                if (shouldAnalyze):
                    ...
                    try:
                        ...
                        logging.error(f"Analysis failed for { fileName }: { str(analysis_error) }")
    ...
    def analyzeImage(cls, filePath, fileName, userId = None):
        ...
        try:
            if (not os.path.exists(filePath)):
                ...
                return flask.jsonify({'message': '找不到要分析的图片文件'}), 404
            ...
            if (fileSize == 0):
                return flask.jsonify({'message': '图片文件为空'}), 400

            if (fileSize > 10 * 1024 * 1024):  # 10MB限制
                return flask.jsonify({'message': '图片文件太大, 请压缩后重试'}), 400
            ...
            try:
                ...
                return flask.jsonify({'message': f'AI客户端初始化失败: { str(client_error) }'}), 500

            dataUrl = cls.imageToDataUrl(filePath)

            if (not dataUrl):
                logging.error(f"Failed to convert image to data URL: { filePath }")
                return flask.jsonify({'message': '图片转换失败'}), 500
            ...
            try:
                ...
                if (not response or not response.choices):
                    ...
                    return flask.jsonify({'message': 'AI分析服务返回空响应, 请稍后重试'}), 500
                ...
                if (not analysisResult or len(analysisResult.strip()) == 0):
                    ...
                    return flask.jsonify({'message': 'AI分析返回空内容, 请稍后重试'}), 500
                ...
                if (userId):
                    try:
                        analysisId = drawing_analysis_manager.save_analysis(
                            user_id = userId,
                            image_path = filePath,
                            analysis_result = analysisResult,
                            image_size = f"{ len(open(filePath, 'rb').read()) } bytes",
