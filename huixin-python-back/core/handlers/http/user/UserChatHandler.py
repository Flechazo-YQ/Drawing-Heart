...
class UserChatHandler:
    ...
    def createNewChat():
        try:
            ...
            return CommonHelper.errorResponse(1, f'创建对话失败: { str(e) }', 500)
    ...
    def hideChat(chatId: str):
        try:
            ...
            if (not success):
                return CommonHelper.errorResponse(1, '隐藏对话失败', 404)

            return CommonHelper.successResponse(0, '隐藏对话成功', 200)
        
        except Exception as e:
            logging.error(f'❌ 隐藏对话失败: { str(e) }')

            return CommonHelper.errorResponse(1, f'隐藏对话失败: { str(e) }', 500)
    ...
    def streamChat():
        ...
        if (not data):
            return CommonHelper.errorResponse(1, '请求数据不能为空!', 400)
        ...
        try:
            ...
            return CommonHelper.errorResponse(1, f'处理用户消息失败: { str(e) }', 500)
    ...
    def getChatMessages(chatId: str):
        try:
            ...
            if (not chat or str(chat['userId']) != userId):
                return CommonHelper.errorResponse(1, '对话不存在或无权限', 404)
            ...
            return CommonHelper.errorResponse(1, f'获取对话消息历史失败: { str(e) }', 500)
    ...
    def getUserChatsList():
        try:
            ...
            return CommonHelper.errorResponse(1, f'获取用户对话列表失败: { str(e) }', 500)
